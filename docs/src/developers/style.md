...
!!! info
    The style guide is always a work in progress, and not all LastHomework code
    follows the rules. When modifying LastHomework, please fix the style violations
...
`LastHomework` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/LastHomework.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `LastHomework` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `LastHomework` have
