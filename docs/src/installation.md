...
[`LastHomework`](https://github.com/singularitti/LastHomework.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("LastHomework")
   ```

3. Run

   ```julia-repl
   julia> using LastHomework
   ```
...
version of `LastHomework`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/LastHomework.jl"
```
...
our [GitHub repository](https://github.com/singularitti/LastHomework.jl)
for new releases.
Once we release a new version, you can update `LastHomework` by typing

```@repl
using Pkg
Pkg.update("LastHomework")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `LastHomework`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("LastHomework")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `LastHomework`.
